    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <a href="/dashboard" className="inline-flex items-center gap-2 bg-[#ED03E9] text-white font-sans font-semibold px-7 py-3 rounded-xl hover:bg-[#a85e35] transition-colors">
            ← Înapoi la dashboard
          </a>
        </div>
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {loading ? (
            <div className="db-card rounded-2xl p-10 text-center text-gray-400 font-sans border border-gray-100">
              <div className="w-8 h-8 border-3 border-[#ED03E9] border-t-transparent rounded-full animate-spin mx-auto mb-3" style={{ borderWidth: 3 }} />
              Se încarcă disponibilitatea...
            </div>
          ) : Object.keys(slots).length === 0 ? (
            <div className="db-card rounded-2xl border border-gray-100 p-10 text-center">
            ...
            <div key={date} className="db-card rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                ...
                {daySlots.map(slot => (
                  <button key={slot.id} onClick={() => setSelected(selected?.id === slot.id ? null : slot)}
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-sans font-semibold border-2 transition-all active:scale-95 ${
                      selected?.id === slot.id
                        ? 'border-[#ED03E9] bg-[#ED03E9] text-white shadow-md'
                        : 'border-gray-200 text-gray-700 hover:border-[#ED03E9]/50 hover:bg-orange-50'
                    }`}>
        ...
        <div className="lg:col-span-1">
          <div className={`bg-white rounded-2xl border shadow-sm p-5 sm:p-6 transition-all ${selected ? 'border-[#ED03E9]/30' : 'border-gray-100'} sticky top-20`}>
            ...
            {selected ? (
              <>
                <div className="bg-gradient-to-br from-[#ED03E9] to-[#B800BA] rounded-xl p-4 text-white mb-4">
                ...
                <div className="mb-4">
                  ...
                  <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 font-sans text-sm focus:outline-none focus:ring-2 focus:ring-[#ED03E9]/40 focus:border-[#ED03E9] resize-none" />
                </div>
                <button onClick={handleBook} disabled={submitting}
                  className="w-full bg-gradient-to-r from-[#ED03E9] to-[#B800BA] text-white font-sans font-semibold py-3.5 rounded-xl transition-all disabled:opacity-60 shadow-md hover:shadow-lg active:scale-95">
